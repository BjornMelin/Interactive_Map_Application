...
neighborhoods <- c("St Anthony Park", "Como", 
                   "North End", "Hamline-Midway",
                   "Macalester-Groveland", "Highland", 
                   "West Side Community Organization", "Summit Hill Association",
                   "Summit-University", "Thomas-Dale/Frogtown", 
                   ...
                   "Payne-Phalen", "The Greater East Side",
                   "Dayton's Bluff", "Eastview-Conway-Battle Creek-Highwood Hills",
                   "Union Park", "Camden", "Northeast", "Near North",
                    "Central", "University",
                    "Calhoun Isle", "Powderhorn", "Longfellow", "Southwest", 
                    "Nokomis", "Phillips")
```
...
```{r, echo=FALSE}
U <- subset(NBHD_MPLS, CommName == "University")

#Como_google <- Como %>% spTransform(CRS("+init=epsg:4326"))

# Creates a plot of the individual neighborhood selected
NBHD_spec <- ggplot() +
  geom_polygon(data = U, aes(x = long, y = lat, group = group),
               color = "blue", fill = NA) + coord_fixed(1.3)
#NBHD_spec

# Makes a map with only the specific neighborhood inputted
# outlined.  Displaying all Nice Ride Stations still
NBHD_map <- ggmap(mpls_map) +
  geom_polygon(data = U, aes(x = long, y = lat, group = group),
               color = "blue", fill = NA) + coord_fixed(1.3) +
  geom_point(data = station_locations_2017, aes(x= Longitude, y = Latitude), 
             size = 1, alpha = 0.75)  +
  coord_fixed(xlim = long, ylim = lat)
NBHD_map
```
...
```{r,echo=FALSE,message=FALSE,warning=FALSE}
    selectInput("plotchoice",
                  ...
                  choices = "Plot 1",
                  selected = "Plot 1")
selectInput("NBHD_choice",
              label = "Select Neighborhood To Display",
              choices = neighborhoods)

    # Main panel for displaying outputs ----
    mainPanel(plotOutput(outputId = "stationplot", width = "100%"),
              position = "center")
```
...
```{r,echo=FALSE,message=FALSE,warning=FALSE}
datasetInput <- reactive({
   city_data <- switch(input$NBHDchoice,
                      "Near North" = "NearNorth",
                      "Calhoun Isle" = "CalhounIsle")
})

output$MPLSplot <- renderPlot({
  
  ds <- subset(input$citychoice, CommName == input$NBHDchoice)
  
  # Makes a map with only the specific neighborhood inputted
# outlined.  Displaying all Nice Ride Stations still
NBHD_map_1 <- ggmap(mpls_map) +
...
output$STPplot <- renderPlot({
  
  ds <- subset(input$citychoice, name2 == input$NBHDchoice)
  
    # Makes a map with only the specific neighborhood inputted
# outlined.  Displaying all Nice Ride Stations still
NBHD_map_2 <- ggmap(mpls_map) +
  geom_polygon(data = ds, aes(x = long, y = lat, group = group),
               color = "blue", fill = NA) + coord_fixed(1.3) +
  geom_point(data = station_locations_2017, aes(x= Longitude, y = Latitude), 
             size = 1, alpha = 0.75)
